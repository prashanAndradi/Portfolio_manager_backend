...
router.post('/ledger-post', checkAuth, checkAdmin, async (req, res) => {
  try {
    ...
    for (const deal of deals) {
      ...
      console.log('Finished posting ledger for deal:', deal.id);
      postedCount++;
    }
    res.json({ success: true, message: `Ledger entries posted for ${postedCount} deals.`, postedCount });
  } catch (err) {
...
router.post('/eod', checkAuth, checkAdmin, async (req, res) => {
  try {
    ...
    if (!systemDayObj) return res.status(400).json({ success: false, message: 'System day not set.' });
    const systemDay = systemDayObj.system_date;
    // Find all deals that need EOD posting for the day
    const deals = await getAllDeals();
    let postedCount = 0;
    for (const deal of deals) {
      // Skip if already posted for this day (add your logic)
      // ...
...
if (isNaN(amount) || amount === undefined) {
  ...
  if (!isNaN(amount)) {
    ...
    // Advance system day (add 1 day)
    const nextDay = new Date(systemDay);
    nextDay.setDate(nextDay.getDate() + 1);
    await setSystemDay(nextDay.toISOString().slice(0, 10));
    res.json({ success: true, message: `EOD complete. Posted for ${postedCount} deals.`, next_system_day: nextDay.toISOString().slice(0, 10) });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
